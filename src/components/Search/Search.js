...
import { getSearch, clearFilm, clearSearch, initializeDirector, initializeFilm } from '../reducers/filmReducer'
...
const SearchPage = () => {
	...
	const handleRemove = (id) => {
		const film = favourites.find(fav => 
			fav.filmId === id
		)
		...
		<List
			...
			renderItem={(item) => (
				<List.Item>
					...
					/>
					<LikeButton id={item.id} handleAdd={handleLike} handleRemove={handleRemove} liked={checkLiked(item.id)}/>
				</List.Item>
			)}
			/>
			: null}
